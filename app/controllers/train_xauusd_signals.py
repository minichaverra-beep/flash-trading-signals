"""
Train ML model on historical XAUUSD (GC=F) M5/H1 data for E1 signal quality.

Usage:
  python -m app.controllers.train_xauusd_signals
  python -m app.controllers.train_xauusd_signals --quick
"""
...
from app.config import PROJECT_ROOT, DATA_DIR, MODELS_DIR

BASE = PROJECT_ROOT
REPORT_PATH = DATA_DIR / "xauusd_ml_training_report.md"

from app.models.xauusd_data import DEFAULT_TICKERS, fetch_xauusd_klines  # noqa: E402
...
M5_CACHE = DATA_DIR / "xauusd_m5.parquet"
H1_CACHE = DATA_DIR / "xauusd_h1.parquet"
ML_SYMBOL = "xauusd"
PRICE_DECIMALS = 2
...
def load_or_fetch_data(days: int, force: bool = False) -> tuple[list[dict], list[dict]]:
    ...
    print(f"Fetching XAUUSD proxy GC=F from yfinance (target ~{days}d)...")
    t0 = time.time()
    m5_bars = min(days * 24 * 12, 8000)
    h1_bars = min(days * 24, 4000)
    m5, h1, meta = fetch_xauusd_klines(
...
def build_snapshot_at_index(
    m5: list[dict], h1: list[dict], idx: int, min_m5: int = 120, require_setup: bool = True
) -> dict | None:
    if idx < min_m5:
        return None
    window = m5[: idx + 1]
    ...
    session = session_flags(ts, asset="XAUUSD")
...
def build_training_dataset(
    ...
    if not rows_x:
        raise RuntimeError("No training samples — check yfinance GC=F data range")
...
def write_report(metrics: dict, meta_df: pd.DataFrame, days: int, horizon: int, elapsed: float) -> None:
    lines = [
        "# ML Training Report — XAUUSD M5 E1 Signals",
        "",
        ...
        f"| SL referencia | ${SL_USD_REF}/oz (~{SL_USD_REF} pts GC=F) |",
        f"| Fuente precios | Yahoo GC=F (proxy XAUUSD) |",
        "",
        f"- Accuracy: {metrics['accuracy']}",
        f"- Precision: {metrics['precision']}",
        f"- Recall: {metrics['recall']}",
        "",
        "**Nota:** yfinance limita intraday; M5 real puede ser < `--days`. "
        "Ops móviles OCR de oro son escasas — ver integración ops.",
        "",
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train XAUUSD M5 E1 signal ML model")
